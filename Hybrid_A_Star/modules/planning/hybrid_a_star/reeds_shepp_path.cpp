...
#include "../../../modules/planning/hybrid_a_star/reeds_shepp_path.h"
...
ReedShepp::ReedShepp(const common::VehicleParam& vehicle_param,
  ...
  std::cout << RED << "max_kappa_ " << max_kappa_ << COLOUR_END << std::endl;
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
  ...
  for (size_t i = 0; i < paths_size; ++i) {
    std::cout << "all_possible_paths.at(i).total_length: " << all_possible_paths.at(i).total_length << std::endl;
  ...
  std::cout << "paths_size: " << paths_size << std::endl;
  std::cout << "optimal_path_index: " << optimal_path_index << std::endl;
...
bool ReedShepp::GenerateRSPs(const std::shared_ptr<Node3d> start_node,
                             const std::shared_ptr<Node3d> end_node,
                             std::vector<ReedSheppPath>* all_possible_paths) {
  bool FLAGS_enable_parallel_hybrid_a = false;
...
bool ReedShepp::SetRSP(const int size, const double* lengths, const char* types,
  ...
  if (path.total_length <= 0.0) {
    std::cout << RED << "total length smaller than 0" << COLOUR_END << std::endl;
    return false;
  }
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  std::cout << "step_scaled:" << step_scaled << std::endl;
...
bool ReedShepp::GenerateRSPPar(const std::shared_ptr<Node3d> start_node,
  ...
  std::cout << RED << "dx :" << dx << COLOUR_END << std::endl;
  std::cout << RED << "dy :" << dy << COLOUR_END << std::endl;
  std::cout << RED << "dphi :" << dphi << COLOUR_END << std::endl;
  std::cout << RED << "c :" << c << COLOUR_END << std::endl;
  std::cout << RED << "s :" << s << COLOUR_END << std::endl;

  std::cout << RED << "x :" << x << COLOUR_END << std::endl;
  std::cout << RED << "y :" << y << COLOUR_END << std::endl;
  std::cout << RED << "xb :" << xb << COLOUR_END << std::endl;
  std::cout << RED << "yb :" << yb << COLOUR_END << std::endl;
...
#pragma omp parallel for schedule(dynamic, 2) num_threads(8)
  for (int i = 0; i < RSP_nums; ++i) {
    ...
    if (tmp_length > 0) {
      if (RSP_param.flag &&
        ...
        std::cout << RED << "Fail at SetRSP, idx#: " << i << COLOUR_END << std::endl;
        succ = false;
      }
    }
  }
  ...
  if (!succ) {
    std::cout << RED << "RSP parallel fails" << COLOUR_END << std::endl;
    return false;
  }
  if (all_possible_paths->size() == 0) {
    std::cout << RED << "No path generated by certain two configurations" << COLOUR_END << std::endl;
    return false;
  }
  return true;
}
