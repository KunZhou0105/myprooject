...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
  ...
  std::cout << GREEN << "(arc) : " << arc << COLOUR_END << std::endl;
  std::cout << GREEN << "(steering) : " << steering << COLOUR_END << std::endl;
  std::cout << GREEN << "(step_size_) : " << step_size_ << COLOUR_END << std::endl;
  std::cout << GREEN << "(traveled_distance) : " << traveled_distance << COLOUR_END << std::endl;
  std::cout << GREEN << "(arc / step_size_) : " << (arc / step_size_) << COLOUR_END << std::endl;
  ...
  std::cout << GREEN << "********************* : " << COLOUR_END << std::endl;
  std::cout << GREEN << "intermediate_x : " << intermediate_x.back() << COLOUR_END << std::endl;
  std::cout << GREEN << "intermediate_y : " << intermediate_y.back() << COLOUR_END << std::endl;
  std::cout << GREEN << "intermediate_phi : " << intermediate_phi.back() << COLOUR_END << std::endl;
  std::cout << GREEN << "********************* : " << COLOUR_END << std::endl;
...
void HybridAStar::CalculateNodeCost(std::shared_ptr<Node3d> current_node,
  ...
  std::cout << "optimal_path_cost : " << optimal_path_cost << std::endl;
...
bool HybridAStar::GetResult(HybridAStartResult* result) {
  ...
  while (current_node->GetPreNode() != nullptr) {
    ...
    if (x.empty() || y.empty() || phi.empty()) {
      std::cout << RED << "result size check failed" << COLOUR_END << std::endl;
      return false;
    }
    if (x.size() != y.size() || x.size() != phi.size()) {
      std::cout << RED << "states sizes are not equal" << COLOUR_END << std::endl;
      return false;
    }
  ...
  if (!GetTemporalProfile(result)) {
    std::cout << RED << "GetSpeedProfile from Hybrid Astar path fails" << COLOUR_END << std::endl;
    return false;
  }
  ...
  if (result->x.size() != result->y.size() ||
    ...
    std::cout << RED << "state sizes not equal, "
           << "result->x.size(): " << result->x.size() << "result->y.size()"
           << result->y.size() << "result->phi.size()" << result->phi.size()
           << "result->v.size()" << result->v.size() << COLOUR_END << std::endl;
    return false;
  }
  if (result->a.size() != result->steer.size() ||
      result->x.size() - result->a.size() != 1) {
    std::cout << RED << " control sizes not equal or not right" << COLOUR_END << std::endl;
    std::cout << RED << " acceleration size: " << result->a.size() << COLOUR_END << std::endl;
    std::cout << RED << " steer size: " << result->steer.size() << COLOUR_END << std::endl;
    std::cout << RED << " x size: " << result->x.size() << COLOUR_END << std::endl;
    return false;
  }
...
bool HybridAStar::GenerateSpeedAcceleration(HybridAStartResult* result) {
  ...
  if (result->x.size() < 2 || result->y.size() < 2 || result->phi.size() < 2) {
    std::cout << RED << "result size check when generating speed and acceleration fail" << COLOUR_END << std::endl;
    return false;
  }
...
bool HybridAStar::GenerateSCurveSpeedAcceleration(HybridAStartResult* result) {
  ...
  if (result->x.size() != result->y.size() ||
      result->x.size() != result->phi.size()) {
    std::cout << RED << "result sizes not equal" << COLOUR_END << std::endl;
    return false;
  }
  ...
  //   std::cout << RED << "Piecewise jerk speed optimizer failed!" << COLOUR_END << std::endl;
  //   return false;
  // }
  ...
  // if (path_data.empty()) {
  //   std::cout << RED << "path data is empty" << COLOUR_END << std::endl;
  //   return false;
  // }
  ...
  //     std::cout << RED << "Fail to get speed point with relative time " << cur_rel_time << COLOUR_END << std::endl;
  //     return false;
  //   }
...
bool HybridAStar::TrajectoryPartition(
  ...
  if (x.size() != y.size() || x.size() != phi.size()) {
    std::cout << RED << "states sizes are not equal when do trajectory partitioning of "
              "Hybrid A Star result" << COLOUR_END << std::endl;
    return false;
  }
  ...
  bool FLAGS_use_s_curve_speed_smooth = false;
  for (auto& result : *partitioned_result) {
    if (FLAGS_use_s_curve_speed_smooth) {
      if (!GenerateSCurveSpeedAcceleration(&result)) {
        std::cout << RED << "GenerateSCurveSpeedAcceleration fail" << COLOUR_END << std::endl;
        return false;
      }
    } else {
      if (!GenerateSpeedAcceleration(&result)) {
        std::cout << RED << "GenerateSpeedAcceleration fail" << COLOUR_END << std::endl;
        return false;
      }
    }
  }
...
bool HybridAStar::GetTemporalProfile(HybridAStartResult* result) {
  ...
  if (!TrajectoryPartition(*result, &partitioned_results)) {
    std::cout << RED << "TrajectoryPartition fail" << COLOUR_END << std::endl;
    return false;
  }
...
bool HybridAStar::Plan(
  ...
  // clear containers
  std::cout << GREEN << "HybridAStar Start" << COLOUR_END << std::endl;
  const auto start_timestamp = std::chrono::system_clock::now();
  open_set_.clear();
  close_set_.clear();
  ...
  std::cout << GREEN << "XYbounds_ === " << XYbounds_.size() << COLOUR_END << std::endl;
  start_node_.reset(
  ...
  if (!ValidityCheck(start_node_)) {
    std::cout << RED  << "start_node in collision with obstacles" << COLOUR_END << std::endl;
    return false;
  }
  if (!ValidityCheck(end_node_)) {
    std::cout << RED << "end_node in collision with obstacles" << COLOUR_END << std::endl;
    return false;
  }
  ...
  std::cout << "start_node_->GetX() : " << start_node_->GetX() << std::endl;
  std::cout << "start_node_->GetY() : " << start_node_->GetY() << std::endl;
  std::cout << "start_node_->GetPhi() : " << start_node_->GetPhi() << std::endl;

  // std::cout << "open_set_.szie() : " << open_set_.size() << std::endl;
  // std::cout << "open_pq_.szie() : " << open_pq_.size() << std::endl;
  // Hybrid A* begins
  ...
  while (!open_pq_.empty()) {
    ...
    std::cout << "open_pq_.size() : "  << open_pq_.size() << std::endl;
    std::cout << "current_id : "  << current_id << std::endl;
    std::cout << "open_set_[current_id]->GetGridX() : " << open_set_[current_id]->GetGridX() << std::endl;
    std::cout << "open_set_[current_id]->GetGridY() : "  << open_set_[current_id]->GetGridY() << std::endl;
    std::cout << "open_set_[current_id]->GetGridPhi() : " << open_set_[current_id]->GetGridPhi() << std::endl;
    std::cout << "open_set_[current_id]->GetCost() : " << open_set_[current_id]->GetCost() << std::endl;
    open_pq_.pop();
    std::cout << "open_pq_.size() : "  << open_pq_.size() << std::endl;
    ...
    for (size_t i = 0; i < next_node_num_; ++i) {
      ...
      std::cout << "next_node->GetGridX() : " << next_node->GetGridX() << std::endl;
      std::cout << "next_node->GetGridY() : " << next_node->GetGridY() << std::endl;
      std::cout << "next_node->GetGridPhi() : " << next_node->GetGridPhi() << std::endl;
      std::cout << "next_node->GetPhi() : " << next_node->GetPhi() << std::endl;
      std::cout << "next_node->GetDirec() : " << next_node->GetDirec() << std::endl;
  ...
  const auto end_timestamp = std::chrono::system_clock::now();
  std::chrono::duration<double> diff = end_timestamp - start_timestamp;
  std::cout << "Hybrid A Star total time: " << diff.count() * 1000.0 << " ms." << std::endl;
