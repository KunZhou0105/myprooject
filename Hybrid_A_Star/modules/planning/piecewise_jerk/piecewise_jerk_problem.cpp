...
#include "../../../modules/planning/piecewise_jerk/piecewise_jerk_problem.h"
...
bool PiecewiseJerkProblem::Optimize(const int max_iter) {
  ...
  if (status < 0 || (status != 1 && status != 2)) {
    std::cout << RED << "failed optimization status:\t" << osqp_work->info->status << COLOUR_END << std::endl;
    osqp_cleanup(osqp_work);
    FreeData(data);
    c_free(settings);
    return false;
  } else if (osqp_work->solution == nullptr) {
    std::cout << RED << "The solution from OSQP is nullptr" << COLOUR_END << std::endl;
    osqp_cleanup(osqp_work);
    FreeData(data);
    c_free(settings);
    return false;
